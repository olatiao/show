(function () {
    ...
    var sidebarClass = function () {
        this.init();
        for (
            var key = 0; key < window.eocourse.sidebarConstant.list.length; key++
        ) {
    ...
    sidebarClass.prototype = {
                        ...
                        default:
                            {
                                ...
                                ).innerHTML = http.responseText.replace(/<(.*)style(.*)>/g, "");
                                break;
                            }
                    }
                };
        ...
        paint: function (arg, elem) {
            ...
            ).test(template.array[arg.level + 1]);
            ...
            var template = {
                html: '<li class="common-level-' +
                    ...
                    arg.level +
                    '" class="' +
                    (isCurrent ? "ico_up" : "ico_down") +
                    '" onclick="eocourse.sidebarClass.click(this)"><a ' +
                    (arg.href ?
                        "onclick=\"eocourse.sidebarClass.router('" +
                        arg.href +
                        "'" +
        ...
        click: function (arg) {
            ...
            if (/hidden/.test(arg.parentElement.getAttribute("class"))) {
                ...
                arg.parentElement.setAttribute(
                    "class",
                    template.class.replace(/hidden/g, "level" + template.level)
                );
            } else {
        ...
        router: function (href, originHref) {
            if (originHref) {
                ...
                window.location.href =
                    window.location.origin + window.location.pathname + "?target=" + href;
            }
        }
    };
